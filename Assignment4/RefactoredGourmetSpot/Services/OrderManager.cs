...
namespace GourmetSpot.Services
{
   public class OrderManager
   {
           ...
           out string message)
       {
           ...
           if (!Save())
           {
               message = GetStorageErrorMessage(
                   "Suborder added, but order data could not be saved.");
               return false;
           }
       ...
       public bool FinalizeTableOrder(
           int tableNumber,
           out Order? order,
           out string message)
       {
           ...
           if (!Save())
           {
               ...
               message = GetStorageErrorMessage(
                   "Table order finalized, but order data could not be saved.");
               return false;
           }
       ...
       private bool CompleteOrderCreation(
           Order order,
           List<OrderItem> selectedMenuItems,
           IInventoryManager inventoryManager,
           bool isFinalized,
           ...
           out string message)
       {
           ...
           if (!inventoryManager.UseIngredients(
               requiredIngredients,
       ...
       private void AddSubOrderToOrder(
           Order order,
           List<OrderItem> selectedMenuItems)
       {
       ...
       private int GetNextSubOrderNumber(Order order)
       {
           ...
           foreach (SubOrder subOrder in order.SubOrders)
           {
               highestSubOrderNumber = Math.Max(
                   highestSubOrderNumber,
