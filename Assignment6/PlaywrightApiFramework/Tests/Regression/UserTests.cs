...
public class UserTests : ApiTestBase
{
    ...
    public async Task CreateUser_WithJsonDataProvider_ShouldValidateBodyAndHeaders(UserRequest user)
    {
        ...
        var contentType = JsonHelper.GetHeader(response, ContentTypes.ContentTypeHeader);
        ...
        ApiAssert.HeaderContains(contentType, ContentTypes.ContentTypeHeader, ContentTypes.ApplicationJson);
        ...
        ApiAssert.GreaterThanZero(userResponse.Id, nameof(userResponse.Id));
    ...
    public async Task CreateUser_WithCsvDataProvider_ShouldValidateBody(UserRequest user)
    {
        var endpoint = UserEndpoints.Users;
    ...
    public async Task JsonContentType_ShouldCreateUser()
    {
        ...
        var response = await UserService.CreateUser(user, ContentTypes.ApplicationJson);
        var userResponse = await JsonHelper.Deserialize<User>(response);
    ...
    public async Task FormDataContentType_ShouldCreateUser()
    {
        ...
        ApiAssert.Status(response, endpoint, HttpStatusCodes.Created);
        ApiAssert.HeaderContains(contentType, ContentTypes.ContentTypeHeader, ContentTypes.ApplicationJson);
        ApiAssert.FieldEquals(userResponse.Name, TestData.FormDataUser["name"], nameof(userResponse.Name));
        ApiAssert.FieldEquals(userResponse.Job, TestData.FormDataUser["job"], nameof(userResponse.Job));
        ApiAssert.GreaterThanZero(userResponse.Id, nameof(userResponse.Id));
    }
    ...
    public async Task ReqResSavesData_ShouldGetCreatedUser()
    {
        ...
        ApiAssert.Status(getResponse, getEndpoint, HttpStatusCodes.Ok, "Expected created ReqRes user id " + createdUser.Id + " to persist, but ReqRes test data is not saved.");
    }
