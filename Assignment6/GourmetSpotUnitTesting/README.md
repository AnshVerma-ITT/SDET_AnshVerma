...
## Assignment 2 Unit Testing Summary

NUnit unit tests have been added in the separate `Tests` folder.
...
Run the NUnit test project from the project root:

```bash
dotnet test Tests/GourmetSpot.Tests.csproj --logger "console;verbosity=detailed"
```

This command prints a test report in the terminal, including the test run status, passed test count, failed test count, skipped test count, and total execution time.
