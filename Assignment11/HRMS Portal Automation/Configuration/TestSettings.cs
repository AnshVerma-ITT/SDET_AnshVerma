...
public sealed class TestSettings
{
    ...
    public static TestSettings Load()
    {
        ...
        {
            throw new FileNotFoundException($"Could not find {SettingsFileName} in the test output directory.", filePath);
        }
    ...
    {
        var configuredPath = Environment.GetEnvironmentVariable(ArtifactsDirectoryVariable) ?? ArtifactsDirectory;
        return Path.IsPathRooted(configuredPath)
            ? configuredPath
            : Path.Combine(TestContext.CurrentContext.WorkDirectory, configuredPath);
    }
    ...
    private void Validate()
    {
        ...
        {
            throw new InvalidOperationException("Timeouts and viewport dimensions must be greater than zero.");
        }

        if (string.IsNullOrWhiteSpace(ArtifactsDirectory) || string.IsNullOrWhiteSpace(TestIdAttribute))
        {
    ...
    private static bool GetBoolean(string variable, bool fallback)
    {
        var value = Environment.GetEnvironmentVariable(variable);
    ...
    private static int GetInteger(string variable, int fallback)
    {
        var value = Environment.GetEnvironmentVariable(variable);
        return string.IsNullOrWhiteSpace(value)
            ? fallback
