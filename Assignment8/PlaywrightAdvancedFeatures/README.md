...
`Tests` contains the advanced Playwright scenarios.

`Fixtures` contains common NUnit setup and teardown, browser creation, trace and video handling, screenshot capture, attachment handling, and project-path creation.

`Configuration` loads and validates runtime values from `appsettings.json`.
...
| --- | --- |
| `Configuration/TestSettings.cs` | Loads and validates JSON test settings. |
| `Fixtures/BrowserTestBase.cs` | Creates and closes Playwright resources and captures evidence. |
| `TestAssets/upload-sample.txt` | File selected by the upload test. |
| `Tests/AdvancedBrowserFeaturesTests.cs` | Contains all advanced browser scenarios. |
| `Reports` | Allure raw result output. |
| `TestResults` | Generated downloads, traces, videos, and failure screenshots. |
...
| --- | --- |
| Downloaded file | `.\TestResults\playwright-example.txt` |
| Traces | `.\TestResults` |
| Videos | `.\TestResults` |
